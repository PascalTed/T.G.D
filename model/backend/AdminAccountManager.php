...
class AdminAccountManager extends Manager
{
    ...
    {
        $db = $this->dbConnect();
        $allAccounts = $db->query('SELECT id, avatar, pseudo, user_right, DATE_FORMAT(registration_date, \'%d/%m/%Y\') registration_date FROM users');
    ...
    {
        $db = $this->dbConnect();
        $req = $db->prepare('SELECT id, email, avatar, pseudo, user_right, DATE_FORMAT(registration_date, \'%d/%m/%Y\') registration_date FROM users WHERE id = ?');
